...
PhyloGeneious is an improved and expanded version of the OrthologID pipeline, optimized for HPC clusters (Slurm and PBS job schedulers).
...
various programs can not only submit jobs but also monitor them for completion. Note that syntax for PBS job submission may vary across systems and need to be adjusted.
...
```sh
git clone -b 2025_update --single-branch https://github.com/coruzzilab/PhyloGeneious.git
cd PhyloGeneious
./setup_license.sh
```
...
--docker=<IMG>      Use the specified docker image for PhyloGeneious
--singularity=<path to IMG>      Use the specified singularity image for PhyloGeneious
```
...
2. config - a configuration file telling basic runtime parameters:
   ...
   - `TREEPROGRAM`= Program to use for gene tree building. TNT=TNT (protein-based) `default`, OBLONG=Oblong+TNT (codon-based) `currently SLURM only`
...
Matrix and partition files can be recreated if some modifications are required using the PhyloGeneious/bin/orth2matrix.pl script (note that it usually requires a lot of memory, so run it on a high memory node (≥128GB)):
```sh
export OID_HOME=/path/to/PhyloGeneious/folder
...
### Data entry issues: 
Issues with data correctness and completeness can cause the pipeline to fail in certain cases. Here are some common issues:

1. Protein sequence files must be in FASTA format. Support for DNA sequences as input is not currently available, though we plan to add this at a later date.

2. The short species identifier (e.g., SPECIES1) must be identical in all three places: 1. the config file where INGROUP and OUTGROUP are specified. 2. the input fasta file name (SPECIES1.faa), and 3. the sequence headers inside the fasta file (e.g., >SPECIES1#seqID) 

### Pipeline customization: 
Compute clusters come in all sizes and configurations, with key differences in job management software, walltime limits, memory limits etc. It's impossible to have the pipeline auto-adjust to all these settings. Please make sure the pipeline is customized for your cluster by doing the following:
...
2. Adjust the resource parameters for your pipeline run in testdata/config.

3. Check that the syntax in job submission commands (e.g. qsub and qstat) matches your system's setup, and modify as appropriate.

### Failure modes: 
The PhyloGeneious pipeline uses an array of software tools in multiple stages. With certain input data sets, especially those that are very large, some of the steps will fail due to resource limitations or other unavoidable issues. 
