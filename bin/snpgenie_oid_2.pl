...
# UPDATED FOR NEW ORTHOLOG SELECTION
...
###	unlink "$OID_USER_DIR/data/$family_id/family$family_id\_between_group_polymorphism.txt";
    ...
    my @protein_aln_files = glob "*\-final\.fasta";    # amino acid alignment

    unless ( scalar(@protein_aln_files) > 0 ) {
        die
"\n\n# There are no *\-final\.fasta amino acid alignment files in family directory\n\n";
    }

    # There is a .aligned file for the FAMILY

    foreach my $protein_aln_file (@protein_aln_files) {
        if ( -f "$protein_aln_file" ) {

            #print "\nWe've got one matching file\n";

            my $partition_id      = '';
            my $middle_characters = '';

            if ( $protein_aln_file =~ /^(.+)_(.+)\-final\.fasta/ )
            { # Gil: the id is assumed to be everything BEFORE the first underscore (_)
                $partition_id      = $1;
                $middle_characters = $2;
            }

            ##################################################################################
# STORE WHICH SPECIES AND MEMBERS ARE IN THIS PARTITION to determine WHICH CDS are needed
            my %family_partition_members_hh;
            my %family_members_h;
            my %family_species_h;
            my %family_partition_species_hh;

            open( IN_PROTEIN_FASTA, "$protein_aln_file" )
              or die "Could not open file $protein_aln_file\n";

            # print "Recording coding sequence data for file $cds_file...\n";

            while (<IN_PROTEIN_FASTA>) {
                chomp;

                my $this_line   = $_;
                my $this_member = $this_line;
                $this_member =~ s/^>//;

                if ( $this_line =~ /^>(.+)#.+/ ) {
                    $family_partition_members_hh{$family_id}->{$partition_id}
                      ->{$this_member} = 1;
                    print
"Added $this_member to family $family_id partition $partition_id\n";
                    ...
                    if ( $family_partition_species_hh{$partition_id}
                        ->{$this_species} == 1 )
                    {
                        warn
"### WARNING: there are multiple members of species $this_species "
                    ...
                    $family_partition_species_hh{$partition_id}->{$this_species}
                      = 1;
                }
            }

            close IN_PROTEIN_FASTA;
...
#			open(BETWEEN_CODON_FILE,">>family$family_id\_between\_group\_codon\_results\.txt");
                ...
                unless ( -f "between\_group\_codon\_results\.txt" ) {
...
"analysis\tfamily\tpartition\tgroup_1\tgroup_2\tcodon\tvariability\t"
                ...
                unless ( -f "between\_group\_results\.txt" ) {
...
"$OID_USER_DIR/data/$family_id/$partition_id\_bootstrap\_results\.txt";
                    unlink
"$OID_USER_DIR/data/$family_id/$partition_id\_$middle_characters\-final\_cds.fasta";
...
"$OID_USER_DIR\/data\/$family_id\/$partition_id\/$partition_id\_between_polymorphic_codons_arr";
                ...
                unless ( -f "this_family_log.txt" ) {
                    print FAMILY_LOG
"family_id\tpartition_id\tnum_codons_in_product\tnum_sequences\tstart_time\tend_time\n";
                }
...
#			close CODON_FILE;
      ...
      # Read in the partition of amino acid sequences from the FAMILY fasta file
                ...
                open( IN_AA_FASTA, "$protein_aln_file" )
                  or die "Could not open file $protein_aln_file\n";
...
"\n\nDIE: The sequences must be aligned, i.e., must be the same length. TERMINATED.\n\n";
            ...
            # Loop through sequences and record number of gaps for each position
                ...
                foreach
                  my $curr_posn ( sort { $a <=> $b } keys %aaPosn_gapCount )
                {
...
#				open(OUT_RETAINED_POSNS, ">>$OID_USER_DIR/data/$family_id\/retained_aa_positions.txt");
...
#				foreach my $curr_header (sort keys %aa_seqs_hash) {
#					my $curr_aa_seq = $aa_seqs_hash{$curr_header};
#
#					my $num_posns_removed = 0;
#					for(my $k = 0; $k < scalar(@posns_to_remove); $k++) {
#						my $posn_to_remove = $posns_to_remove[$k] - $num_posns_removed; # to account for offset due to already-removed positions
...
#
#					#&print_fasta_file("partition$partition_id\_aligned.faa",$curr_header,$curr_aa_seq);
#					print_fasta_file("$partition_id\_$middle_characters\_aligned.faa", $curr_header, $curr_aa_seq);
#
#				}
         ...
         # IMPOSE the amino acid ALIGNMENTS on the relevant nucleotide sequences
                ...
                my @aligned_seqs_arr
                  ;    # this will be of same length as %aa_seqs_hash
                my @sorted_aa_seqs_hash_keys =
                  sort { $a <=> $b } keys %aa_seqs_hash;

                for (
                    my $i = 0 ;
                    $i < scalar(@sorted_aa_seqs_hash_keys) ;
                    $i++
                  )
                {
                    my $this_aln_aa_seq_header = $sorted_aa_seqs_hash_keys[$i];
                    my $this_aln_aa_seq =
                      $aa_seqs_hash{$this_aln_aa_seq_header};
...
#			my $this_nt_seq_aligned = &align_codon2aa($this_aln_aa_seq,$this_nt_seq,$this_aln_aa_seq_header);
                  ...
                  #				print "\n\nMy aligned sequence:\n$this_nt_seq_aligned\n";
                    ...
                    {    # if unequal lengths, etc., nothing returned
                        ...
                        print_fasta_file(
"$partition_id\_$middle_characters\-final\_cds.fasta",
           ...
           #print CODON_FILE "$family_id\t$partition_id\t" . $codon_num . " \t";
                      ...
                      # ADD: minimum frequency/codon in pairwise deletion method
                        ...
                        {    # if codon is polymorphic, value is 1
                            ...
                            my $this_actual_count =
                              $codonIndex2count[$codon_index];
                          ...
                          #print CODON_FILE "polymorphic\t$this_actual_count\t";
                            ...
                            {    # for each si sequence at this codon
                                my $codon_si = $codonum_codon_aa[$codon_index]
                                  ->[$si_seq_index];
...
#print "\nmy group name i is: $group_name_i\nmy group name j is: $group_name_j\n";
                        ...
                        # the fasta file corresponding to group j is:
                                ...
                                # Updated here:
                                my $num_seqs_per_codon_groupi =
                                  scalar @group_1_spp;
                                my $num_seqs_per_codon_groupj =
                                  scalar @group_2_spp;
...
#print "\ngroup_1_species is $group_1_species group_2_species is $group_2_species\ncodon_gi is $codon_gi codon_gj is $codon_gj\n";
              ...
              #								$poly_site_data_hh{$codon_index}->{$codon_sj} += $weight;
                             ...
                             #								print CODON_FILE "($codon_gi\-$codon_gj)";
                                                    ...
                                                    my $codon_gi_N_sites = (
                                                        $codon_gi_sites_1_arr[
                                                          0] +
                                                          $codon_gi_sites_2_arr[
                                                          0] +
                                                          $codon_gi_sites_3_arr[
                                                          0]
                                                    );
                                                    my $codon_gi_S_sites = (
                                                        $codon_gi_sites_1_arr[
                                                    ...
                                                    my $codon_gj_N_sites = (
                                                        $codon_gj_sites_1_arr[
                                                          0] +
                                                          $codon_gj_sites_2_arr[
                                                          0] +
                                                          $codon_gj_sites_3_arr[
                                                          0]
                                                    );
                                                    my $codon_gj_S_sites = (
                                                        $codon_gj_sites_1_arr[
...
"\n### WARNING: between-group analysis requested but no file given. Skipping analysis.\n";
                    ...
                    if ( $product_N_sites_sum > 0 ) {
                        $product_piN =
                          $product_N_diffs_sum / $product_N_sites_sum;
                    }
                    else {
                        $product_piN = '*';
                    }
                    ...
                    if ( $product_S_sites_sum > 0 ) {
                        $product_piS =
                          $product_S_diffs_sum / $product_S_sites_sum;
                    }
                    else {
                        $product_piS = '*';
                    }
                ...
                #		open(VARIANT_FILE,">>within\_family\_variant\_results\.txt");
                    ...
                    foreach my $codon_index ( sort { $a <=> $b }
                        keys %poly_site_data_hh )
                    {
...
"$family_id\t$partition_id\t$codon_num\t$consensus_codon\t$num_alleles\t"
                        ...
                        $min_count2num_alleles{$min_codon_count}
                          ->{$num_alleles}++;
