...
## now spin and wait for the jobs to complete
   ...
   for (my $prefix=1;$prefix<=$parts;$prefix++){
       open (FB,">$Blastdir/Part$prefix.faa") or die "couldn't open Part$dummypart.faa\n";
       my $rotlen = 0;
       my $rused = 0;
...
#           $rotlen += length($_);
   ...
   open (FA,">$Blastdir/.Part.done");  # mark sucessful completion of making parts so it isn't redone
   close FA;
