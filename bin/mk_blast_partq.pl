...
my $dummypart = 1000+$outsfx;
       ...
       print "blastdb output to $Blastdir combined $comb tasks $MAXQS qsubs\n";
    ...
    ## now get cute
       close  FA;
       open (FB,">$Blastdir/Part$dummypart.faa") or die "couldn't open Part$dummypart.faa\n";
       open (FA,"$comb") or die "could not find $comb\n";
       my $rotlen = 0;
       $curfa = '';
       my $rused = 0;
       ...
       $t = localtime;
           ...
           my $rc = system("rm $Blastdir/.?*$dummypart.done");
           runBlast($dummypart);
       my $ct = localtime;
       ...
       print "time $dif sec - need about $recforhr records/ len $sizforhr to run $MINBL min\n";
       my $parts =  ceil($totlen/$sizforhr);
       my $dbs = @Blstq::dbfiles;
       my $thrash = ($parts/$MAXQS)*$MINBL;  # This is aprox total time with above calc
       my $thrhr = $thrash/60;

       print "have $dbs differend blast dbs and need $parts  parts for $MINBL min runs\n";
...
##       if ($dbs>12 and $parts < $dbs){
       if ($secbl/$dbs<300 and $thrash>60) {
           ...
           print "above value would thrash cache as each blast would run ",$secbl/$dbs," seconds\n";
           print "better: $parts parts: each runs for ",5*$dbs," min : size $sizforhr, recs $recforhr\n";
           $thrash = ($parts/$MAXQS*300*$dbs/3600);
           print "this vsn may do better than $thrash hrs\n";
       }
       $sizforhr = ceil($sizforhr);
       $rc = system("rm $Blastdir/Part$dummypart.faa");
