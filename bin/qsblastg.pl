...
die "qsblastq.pl noblastfiles (-g nn -w nn -q nn -n cpus) gb,wall and max qsubs cpus for blast\n" if $help;
...
for (my $bl=1;$bl<= $nofbl;$bl++){
    ...
    next if ( -f "$Blastdir/.$bl.done" );  # did this guy aready
    next if ( $actblst{$bl});
    if ($qsct >= $MAXQS) {
