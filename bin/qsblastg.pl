...
my $Checkpoint = "$OID_USER_DIR/chkqsblastq.log";
my $Chkfa;
my $Chkcnt = 0;
my %chktbl = ();
...
sub startqs {    #starts a qsub
    ...
    my $JOB_SCRIPT = "$OID_USER_DIR/run_blastq_job.sh";

    #    my $JOB_SCRIPT="$OID_HOME/bin/run_blastq.pl";
...
die
"qsblastq.pl noblastfiles (-g nn -w nn -q nn -n cpus) gb,wall and max qsubs cpus for blast\n"
  if $help;
...
if ( $nofbl < 1 ) {
  ...
  #         my $pid = open(GR, qw/grep -c "$srch" $Blastdir/." 2>/dev/null | ");
    my $grst = "ls $Blastdir | grep -c '$srch'  ";
    print "$grst\n";
    my $pid = open( GR, "$grst |" );
    $nofbl = <GR>;
    chomp $nofbl;
...
for ( my $bl = 1 ; $bl <= $nofbl ; $bl++ ) {
    $blastp++;
    print "look for $Blastdir/.$bl.done\n";
    next if ( system( "grep -q '\\.$bl\\.done' $Blastdir/.Parts.done") < 1 );    # did this guy aready
    next if ( $actblst{$bl} );
    if ( $qsct >= $MAXQS ) {
...
while (1) {
    ...
    $t = localtime;
    $qsct--;
    if ( system( "grep -q '\\.$bl\\.done' $Blastdir/.Parts.done") < 1 ) {
        ...
        while (1) {    #needed because we may have to traverse busy blasts
            ...
            $bl = $blastp;
            next if ( system( "grep -q '\\.$bl\\.done' $Blastdir/.Parts.done") < 1 );
            next if ( $actblst{$bl} );
            $qsubed = 0;
            until ($qsubed) {
                if ( startqs( \%qsid, $bl, $wall ) ) {
...
#my  $rc = system("$OID_HOME/bin/Blstq.pl -B");
combBlast($nofbl);
$t = localtime;
print "$t ran combBlast\n";
