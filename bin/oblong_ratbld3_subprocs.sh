	...
	if [[ ! -f $out/oid.tre ]]; then
		for ((j=0;j<5;j++)); do
			...
			if [[ ! -f $out/${prefix}${k}.rat ]]; then
				...
				#collapse rule 3?
				#. as character?
				$oblong_exe -f -m4000 -R${RAND} -i${rtFamily} -spr -o${prefix}${k}temp1 2>>${out}/oblong.log;
				...
				for ((i=1;i<=iter;i++)); do
					if [[ -f ${prefix}${k}temp3 ]]; then
						$oblong_exe -f -m4000 -i${rtFamily} -T${prefix}${k}temp3 -x1*10 -j37 -spr -o${prefix}${k}temp2 2>>${out}/oblong.log;
					else
						$oblong_exe -f -m4000 -i${rtFamily} -T${prefix}${k}temp1 -x1*10 -j37 -spr -o${prefix}${k}temp2 2>>${out}/oblong.log;
					fi
					...
					$oblong_exe -f -m4000 -i${rtFamily} -T${prefix}${k}temp2 -spr -o${prefix}${k}temp3 2>>${out}/oblong.log;
				...
				awk '/OPTIONS/{exit} 1' ${prefix}${k}temp3 > $out/${prefix}${k}.rat
