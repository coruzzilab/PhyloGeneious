...
print "datadir $OID_DATADIR program $TREEPROGRAM\n";
print "maxqs $MAXQS\n";
print "local time $t\n";
...
for ( my $redo = 0 ; $redo < 2 ; $redo++ ) {
    for $fam (@taxsz) {
        ...
        chdir "$OID_DATADIR/$fam";
        next if ( -s "oid.tre" );    #finished
        next
          if ( $sztaxa{$fam} < $TNTB )
          ;    #could use last, but this is safer if strange order
        next if ( $actfam{$fam} );
        next
          if ( $rcdir{$fam} )
          ;    #already running  - if we do a second pass important
        my @parms = ();
        if ( -s "oid.nex" ) {
            open( NX, "oid.nex" );
            $_ = <NX>;
