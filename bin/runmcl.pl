...
sub startqs {    #starts a qsub
    ...
    ( my $JOB_SCRIPT, my $rqs, my $gbx, my $blid, my $ncpux, my $wl, my $parg )
      = @_;
...
while ( !-f $mcxdone ) {    # need to do mcxload
    ...
    if ( $memavail >= $needmem and ( ( $td + $needtm ) < 72000 ) ) {
        print
"need $needtm time and $needmem GB have $memavail gb and enough time so run mcx inline\n";
        mclmcx();
    }
    else {
...
while ( !-f $familydone ) {
    ...
    my $wl      = 3;                 #shoud be  < 2 hrs
    my @args;
    push @args, $nthr;
    $qsubed = 0;
    until ($qsubed) {
        if (
            startqs( $mclfam_script, \%qsid, $needmem, 2, $nthr, $wl, \@args ) )
        {
            $qsubed = 1;
        }
    }
