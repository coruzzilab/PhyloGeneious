...
my $procs_per_node;
##my $min_freq_in_alignment = 0; # the min. freq. of a codon in an alignment to be considered (prop., not percentage)
##my $min_count_in_alignment = 1; # the min. count of a codon in an alignment to be considered (prop., not percentage)
...
#			"min_freq_in_alignment:f" => \$min_freq_in_alignment, # optional floating point parameter
#			"min_count_in_alignment:i" => \$min_count_in_alignment, # optional integer parameter
...
#print "\nMaximum number of parallel processes to be invoked is $procs_per_node\n";
...
##if(! $min_freq_in_alignment) {
##	if($min_freq_in_alignment != 0) {
##		$min_freq_in_alignment = 0; # DEFAULT is 0
##	}
###	$param_file_contents .= "MINIMUM ALLELE FREQUENCY: Default used; all SNPs included\n";
##} elsif(($min_freq_in_alignment >= 1) || ($min_freq_in_alignment < 0)) {
##	die "\n## WARNING: The --min_freq_in_alignment option must be a decimal between 0 and 1\n".
##		"## SNPGenie for OID terminated.\n\n";
##} #else {
###	$param_file_contents .= "MINIMUM ALLELE FREQUENCY: $minfreq\n";
###}
##
##if(! $min_count_in_alignment) {
##	if($min_count_in_alignment != 0) {
##		$min_count_in_alignment = 1; # DEFAULT is 1 sequence
##	}
##} elsif($min_count_in_alignment < 1) {
##	die "\n## WARNING: The --min_count_in_alignment option must be a decimal between 0 and 1\n".
##		"## SNPGenie for OID terminated.\n\n";
##} #else {
###	$param_file_contents .= "MINIMUM ALLELE FREQUENCY: $minfreq\n";
###}
...
##		$num_bootstraps = 1000; # default behavior
##	}
###	$param_file_contents .= "SLIDING WINDOW LENGTH: Default used; 9 codons\n";
...
#### Get the numbers of all ortholog families
...
# POSSIBLY ADD: parallelize this; must postpone printing all results to summary file; loop through all afterward

#########################################################################################
# LOOP through each family, i.e. directory in /data/
...
# MOVE THIS TO UNDER THE NEXT ELSE...?
...
###	unlink "$OID_USER_DIR/data/$family_id/family$family_id\_between_group_polymorphism.txt";
    ...
    #		print "Working on family $family_id...\n";

    #		print "\nMy matching files are: @matching_files\n";
    ...
    if ( scalar(@matching_files) == 1 ) {
        ...
        #			print FAMILY_LOG "$family_id\t";

    #			open(CODON_FILE,">>within\_family\_codon\_results\.txt");
    #			print CODON_FILE "family\tpartition\tcodon\tvariability\tcomparisons\t".
    #				"N_sites\tS_sites\tN_diffs\tS_diffs\n";
    #			close CODON_FILE;

#		open(VARIANT_FILE,">>within\_family\_variant\_results\.txt");
#		print VARIANT_FILE "family\tcodon_num\tconsensus_codon\tnum_alleles\t".
...
#			print OUTFILE_BETWEEN_SW "analysis\tfamily\tpartition\tgroup_1\tgroup_2\twindow\tfirst_codon\tlast_codon\t".
...
#					"Sequence ignored in dN/dS analysis.\n\n";
            ...
            if ( !@codonum_codon_aa ) {
                ...
                #PARALLELIZE
                mkdir("$partition_id\_polymorphic_codons_arr");
                chdir("$partition_id\_polymorphic_codons_arr");
...
#							}
                    ...
                    print THIS_POLY_TEMP_FILE
                      "$this_codon_poly\t$this_codon_num_seqs_defined";
...
###					$num_seqs_defined[$codon_index] = $this_codon_num_seqs_defined;
                ...
                $pm_poly
                  ->wait_all_children;    # special name, methods within module

                my @polymorphic_codons_FILES_arr = glob "*";
                @polymorphic_codons_FILES_arr =
                  sort { $a <=> $b } @polymorphic_codons_FILES_arr;
   ...
   #print "sorted polymorphic_codons_FILE_arr: @polymorphic_codons_FILES_arr\n";
                ...
                chdir("..");
                rmdir("$partition_id\_polymorphic_codons_arr");

                # Finish parallelism for polymorphism detection

#print "\nDone recording polymorphism in $family_id\nIt is: @polymorphic_codons_arr\n\n";

#				# ADD: determine whether to use minimum frequency or minimum count parameter
#				my $this_min_count;
#
#				#print "\nMin freq is: $min_freq_in_alignment\nMin count is: $min_count_in_alignment\n\n";
#
#				# If both have been specified
#				if($min_freq_in_alignment > 0 && $min_count_in_alignment > 0) { # Determine which is less stringent, minimum count or frequency
#					#print "\nGot here 1\n";
#					my $minFreq2count = $min_freq_in_alignment * $num_seqs;
#
#					if($minFreq2count < $min_count_in_alignment) {
#						$this_min_count = $minFreq2count;
#					} else {
#						$this_min_count = $min_count_in_alignment;
#					}
#				} elsif($min_freq_in_alignment > 0) { # OR IF JUST FREQUENCY, DEFINE ITS MIN COUNT
#					#print "\nGot here 2\n";
#					$this_min_count = $min_freq_in_alignment * $num_seqs;
#				} elsif($min_count_in_alignment > 0) { # OR IF JUST FREQUENCY, DEFINE ITS MIN COUNT
#					#print "\nGot here 3\n";
#					$this_min_count = $min_count_in_alignment;
#				} else {
#					#print "\nGot here 4\n";
...
#print CODON_FILE "$mean_N_sites\t$mean_S_sites\t$mean_N_diffs\t$mean_S_diffs\n";
          ...
          #							}
          #						} # we have a sufficient number of defined codons
...
# Quickly see if it's polymorphic between-group to save time. It's faster just to check, EVEN IF not polymorphic
                            ...
                            mkdir(
                                "$partition_id\_between_polymorphic_codons_arr"
                            );
                            chdir(
                                "$partition_id\_between_polymorphic_codons_arr"
                            );
...
#			my $procs_poly = 10; # number to do at once, cores to assign simultaneously, machine-limited. CUVIER has 80
                            my $pm_poly =
                              Parallel::ForkManager->new($procs_per_node);
...
#	"species $group_1_species vs. $group_2_species\ncodon $codon_gi vs. $codon_gj\n";
                                ...
                                ##				push(@between_polymorphic_codons_arr,$this_codon_poly);
                                ##
                            ...
                            ##
                            $pm_poly->wait_all_children
                              ;    # special name, methods within module
   ...
   #print "sorted polymorphic_codons_FILE_arr: @polymorphic_codons_FILES_arr\n";
                            ...
                            foreach (@polymorphic_codons_FILES_arr)
                            {      # file names, actually
                                my $poly_file = $_;
...
"\n## Cannot open $poly_file. TERMINATED.\n\n";
                                while (<CURR_POLY_FILE>) {
                                    chomp;
                                    push( @between_polymorphic_codons_arr, $_ );
                                }
                                close CURR_POLY_FILE;
                                unlink $poly_file;
                            }
...
###							print POLY_FILE "\nDone recording polymorphism in partition $partition_id between groups\n$group_name_i\nand\n$group_name_j\nIt is: @between_polymorphic_codons_arr\n\n";
###							close POLY_FILE;
...
"\nAnalyzing polymorphic codons in partition $partition_id\...\n";
          ...
          # INTRO: skip if not poly, just do values for the first codon observed
                            ...
                            # PARALLEL
                            mkdir("$partition_id\_codon\_analysis")
                              ;    #?needed/used?
                            chdir("$partition_id\_codon\_analysis")
                              ;    #?needed/used?
...
#							my %between_poly_site_data_hh; # $poly_site_data_hh{codon_index}->{unique_variants}
...
##							$pm_codons->wait_all_children; # special name, methods within module
                            ...
                            @codon_analysis_FILES_arr =
                              sort { $a <=> $b } @codon_analysis_FILES_arr;

   #print "sorted polymorphic_codons_FILE_arr: @polymorphic_codons_FILES_arr\n";

                            foreach (@codon_analysis_FILES_arr)
                            {    # file names, actually
                                my $codon_file = $_;    # this is the INDEX

                                open( CURR_CODON_FILE, $codon_file )
                                  or die
...
##										$codonIndex_conserved{$codon_file} = $this_line_arr[7]; # COMPARISONS column
                                        # PROBLEM
                                    }
                                    else {
                                        die
"\nPROBLEM: site neither conserved nor polymorphic!? DIE\n\n";
                            ...
                            chdir("..");
                            rmdir("$partition_id\_codon\_analysis");

        # done sweeping output <-- I think we abandoned parallelism here for now
...
#
#
# 								# THIS COULD BE THE ACTUAL THING TO CHANGE...
# 							#	my $first_codon = $codon_index + 1;
...
# 										$window_N_diffs += $codon_data_hh{$i}->{N_diffs};
# 										$window_S_diffs += $codon_data_hh{$i}->{S_diffs};
# 										$window_N_sites += $codon_data_hh{$i}->{N_sites};
# 										$window_S_sites += $codon_data_hh{$i}->{S_sites};
# 									}
#
# 									my $window_dN;
# 									if ($window_N_sites > 0) {
...
# ##									if($num_bootstraps > 1) {
# ##										# MAKE BOOTSTRAP FILE
# ##							##			open(BOOT_FILE_WINDOW,">>bootstrap_window_$window_num\.txt");
# ##							##			print BOOT_FILE_WINDOW "file_name\twindow_num\tbootstrap_num\tsim_N_sites_sum\t".
# ##							##					"sim_S_sites_sum\tsim_N_diffs_sum\tsim_S_diffs_sum\n";
# ##
# ##										#################
# ##										# BOOTSTRAPPING TO CALCULATE STANDARD ERROR HERE?
# #####										print "BOOTSTRAPPING for SE(dN-dS)... ";
# ##
...
# ##
# ##								##		# PARALLELIZE IT
# ##								##		mkdir("$window_name\_bootstrap_temp_files");
# ##								##		chdir("$window_name\_bootstrap_temp_files");
# ##								##		#my $procs = 60; # number to do at once, cores to assign simultaneously, machine-limited. CUVIER has 80
# ##								##		my $pm = Parallel::ForkManager->new($num_parallel_procs);
# ##
...
# ##												# For example, if $first_codon==2 and $last_codon==10, we have:
# ##												# int(rand(10-2+1)) + 2
# ##												# int(rand(9)) + 2
# ##												# MIN: 0 + 2 = 2
# ##												# MAX: 8 + 2 = 10
# ##
# ##												my $sampled_N_diffs = $codon_data_hh{$random_codon_num}->{N_diffs};
# ##												my $sampled_S_diffs = $codon_data_hh{$random_codon_num}->{S_diffs};
# ##												my $sampled_N_sites = $codon_data_hh{$random_codon_num}->{N_sites};
# ##												my $sampled_S_sites = $codon_data_hh{$random_codon_num}->{S_sites};
# ##
...
# ##							##				my $out_line_boot = "$codon_file_name\t$window_num\t$bootstrap_num\t$sim_N_sites_sum\t".
# ##							##					"$sim_S_sites_sum\t$sim_N_diffs_sum\t$sim_S_diffs_sum\n";
# ##
# ##							##				print BOOT_FILE_WINDOW "$out_line_boot";
# ##
# ##											#print "$out_line_boot";
# ##
...
# ##										}
# ##
# ##							#			print "\nwindow SE(dN-dS) = $SE_dN_minus_dS\n".
# ##							#				"window Z-value = $window_boot_Z\n\n";
# ##
...
# 										$window_num_defined_codons_g1 += $codonum_group_numDefinedCodons_hh{$j}->{group1};
#
# 										if($codonum_group_numDefinedCodons_hh{$j}->{group1} < $min_defined_codons_in_window) {
# 											$min_defined_codons_in_window = $codonum_group_numDefinedCodons_hh{$j}->{group1};
# 										}
# 									}
#
# 									my $window_num_defined_codons_g2 = 0;
# 									for(my $j = $first_codon; $j <= $last_codon; $j++) {
# 										$window_num_defined_codons_g2 += $codonum_group_numDefinedCodons_hh{$j}->{group2};
#
# 										if($codonum_group_numDefinedCodons_hh{$j}->{group2} < $min_defined_codons_in_window) {
# 											$min_defined_codons_in_window = $codonum_group_numDefinedCodons_hh{$j}->{group2};
# 										}
# 									}
#
...
#
# 									my $out_line_between_sw = "$analysis_name\t$family_id\t$partition_id\t$group_name_i\t$group_name_j\t".
...
# #										}
# #
# #										$out_line_between_sw .= "\t$SE_dN_minus_dS\t$window_boot_Z\t$significance\n"; #####
# #
# #									} else {
# 										$out_line_between_sw .= "\n";
# #									}
...
# 							#		print "ACTUAL:\n$out_line_between_sw\n\n";
#
# #									open(OUTFILE_BETWEEN_SW,">>$sw_file_name");
# 									open(OUTFILE_BETWEEN_SW,">>between\_group\_sw_" . $sliding_window_size . "codons\_results.txt");
# #									if ($first_print == 0) {
# #										$first_print += 1; # Increment so header is only printed once
# #										print OUTFILE_BETWEEN_SW "partition\twindow\tfirst_codon\tlast_codon\t".
# #											"window_N_diffs\twindow_S_diffs\t".
# #											"window_N_sites\twindow_S_sites\t".
# #											"dN\tdS\t".
# #											"dN\-dS\t".
# #											"dN\/dS\tdN\>total_dS";
# #
...
# 				##########################################################################
# 				# OPTIONAL BOOTSTRAPPING (WITHIN-GROUP)
# 				my $SE_dN_minus_dS = 0;
# 				my $product_boot_Z = 'NA';
#
# 				if($num_bootstraps > 1) {
# 					# MAKE BOOTSTRAP FILE
# ##					open(BOOT_FILE_PRODUCT,">>$partition_id\_bootstrap\_results\.txt");
# ##					print BOOT_FILE_PRODUCT "partition\tbootstrap_num\tsim_product_N_sites_sum\t".
# ##							"sim_product_S_sites_sum\tsim_product_N_diffs_sum\tsim_product_S_diffs_sum\n";
#
# 					#################
# 					# BOOTSTRAPPING TO CALCULATE STANDARD ERROR HERE?
# 					print "\n\n#### BOOTSTRAPPING for SE(dN-dS) ####\n\n";
#
# 					#print "num_codons_in_product: $num_codons_in_product\n";
#
# 					my @sim_N_sites_arr;
...
#
# 						# Print product totals for BOOTSTRAP
# ##						my $out_line_boot = "$partition_id\t$bootstrap_num\t$sim_product_N_sites_sum\t".
# ##							"$sim_product_S_sites_sum\t$sim_product_N_diffs_sum\t$sim_product_S_diffs_sum\n";
#
...
# 					my $actual_dN = '*';
# 					my $actual_dS = '*';
# 					my $actual_dN_minus_dS = '*';
#
# 					if($product_N_sites_sum > 0) {
# 						$actual_dN = $product_N_diffs_sum / $product_N_sites_sum;
# 					}
#
# 					if($product_S_sites_sum > 0) {
# 						$actual_dS = $product_S_diffs_sum / $product_S_sites_sum;
# 					}
#
# 					if($actual_dN >= 0 && $actual_dS >= 0) {
# 						$actual_dN_minus_dS = $actual_dN - $actual_dS;
# 					}
#
#
# 					# CALCULATE BOOTSTRAP STANDARD ERROR HERE! NEI & KUMAR (2000) EQUATIONS
...
#
# 					if($SE_dN_minus_dS > 0) {
# 						$product_boot_Z = $actual_dN_minus_dS / $SE_dN_minus_dS;
# 					}
#
# ###					print "\nproduct SE(dN-dS) = $SE_dN_minus_dS\n".
# ###						"product Z-value = $product_boot_Z\n\n";
#
# 				} # END WITHIN-GROUP BOOTSTRAPS
 ...
 # my $num_seqs = scalar @{$codonum_codon_aa[0]};
                        ...
                        sort keys %{ $poly_site_data_hh{$codon_index} } )
                    {
                        ...
                        #				$num_alleles ++;
                        my $curr_codon_count =
                          ( $poly_site_data_hh{$codon_index}->{$codon_triplet}
...
###	$pm_family->finish; # special name
###} # Done with all families (i.e., numbered directories in $OID_USER_DIR/data)

###$pm_family->wait_all_children;
