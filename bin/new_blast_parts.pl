...
my $reportfl = "$OID_USER_DIR/mk_part$outsfx";
       ...
       my $TCPU = $OrthologID::NCPU;   #cpu's to start for blast
       $HPC = $OrthologID::HPC; #env nogood
       my $secbl = $MINBL*60;
       print "I will try to run blasts at $secbl secs ($MINBL min) avg time\n";
       if ( -f "$OID_BLAST/.Part.done"){
           print "$OID_BLAST/.Part.done exists...\n";
...
## first run 2 dummy blasts to compute sizes
       my $run_shell = "$OID_HOME/bin/run_mk_blast".$HPC.".sh";
       print "$run_shell\n";
       for (my $i=1;$i<=$runs;$i++) {
           my $reportflx  = "$OID_USER_DIR/mk_part$i";
           unlink $reportflx;
#           system ("$OID_USER_DIR/qstest.sh $i");
#           system ("$OID_HOME/bin/run_mk_blast.sh $i $TCPU");
           my $rc = system ("$run_shell $i $TCPU");
...
## now spin and wait for the jobs to complete
       ...
       while($fndfile){
           if (-s $reportfl) {
               ...
               if ($fndfile>0){
                   $outsfx++;
                   $reportfl  = "$OID_USER_DIR/mk_part$outsfx";
                   print "looking for $reportfl\n";
...
#       $ptsz = ceil($ptsz/$runs);
       $ptsz = ceil($ptsz/$parts);
       $ptrec = ceil($ptrec/$runs);
       $ptdif = $ptdif/$runs;
#       my $wc = open(WC,"wc $comb |");
#       $_ = <WC>;
#       print "result of wc $_";
#       chomp;
#       @stuff = split/\s/;
#       if ($#stuff >1 and $stuff[2]>0){
#           $ptsz = $stuff[2]/$parts;
#           print "actual $comb size $stuff[2] so us $ptsz to divide parts\n";
#       }
   ...
   for (my $prefix=1;$prefix<=$parts;$prefix++){
       open (FB,">$OID_BLAST/Part$prefix.faa") or die "couldn't open Part$dummypart.faa\n";
       my $rotlen = 0;
       my $rused = 0;
       ...
       while (<FA>){
           ...
           if(/^>/){
               if ($curfa){
                   if (! @fatxt or ($fatxt[0] =~ /^X+$/ and $fatxt[$#fatxt] =~ /^X+$/)) {
...
#           $rotlen += length($_);
   ...
   open (FA,">$OID_BLAST/.Part.done");  # mark sucessful completion of making parts so it isn't redone
   close FA;
