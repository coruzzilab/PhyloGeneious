...
sub HELP_MESSAGE() {
	print "Usage: orth2matrix.v2.pl [ -O ] [ -m min_non_missing_taxa ] [ -x comma_delimited_taxon_list_to_exclude ]\n";
	print "Setting the -O flag will remove all partitions that do not contain a sequence from the outgroup. Default: off\n";
	print "-m sets the minimum number of taxa that need to be represented in a partition to include it. Default: 4\n";
	print "-x may be used to exclude taxa from the final matrix. Default: None\n";
}
...
open MFH, ">$MatrixFile" or die "Cannot open file for writing PAUP matrix: $!\n";
print MFH "$matrix\n";
...
open TNT, ">$TNTMatrixFile" or die "Cannot open file for writing TNT matrix: $!\n";
open PHY, ">$LikelihoodMatrixFile" or die "Cannot open file for writing Likelihood matrix: $!\n";
open PART, ">$PartitionFile" or die "Cannot open file for writing partition length information: $!\n";
open PSET, ">$PartitionMembers" or die "Cannot open file for writing partition membership information: $!\n";
...
sub code {
	...
	$part++;
	if($output eq ""){print LOG "Partition $part: $partition is empty\n";}
	return($output);
}
