...
my $t = localtime;
my %param =
  ( 'c' => 0, 'h' => 1, 'i' => 200, 'm' => 'oid.nex', 'n' => 0, 'w' => 10 );
...
if ( $c and $n and -s "$m" )
{    #mandatory parameters - should be in data subdirectory to run
    ...
    for ( my $k = $n ; $k > 0 ; $k-- ) {
        ...
        for ( my $j = $i ; $j > 0 ; $j-- ) { ############################### RAT
            ...
            $buffer .= "save.;\n";
            $buffer .=
              "quote ------------------ EQUAL WEIGHT SEARCH $j of $i;\n";
            $buffer .= "cc [ .;\n";
            $buffer .= "xi;\n";
            $buffer .= "riddup;\n";
            $buffer .= "bbreak=tbr;\n";
        ...
        $buffer .= "zz;\n";
        $buffer .= "proc/;\n";
        open( OUTFILE, ">oi$prfil.proc" )
          || die("could not open oi$prfil.proc\n!");
    ...
    print(
"USAGE: ratchet.pl -c characters [-h hold] [-i iterations] -m matrix.ss -n files\n[-w \%weight]\n\n"
    );
    print("WHERE\n");
