...
# PROGRAM: Recalculate whole-partition and sliding window dN/dS values FOR BETWEEN-GROUP
# 		ANALYSES using a given window and step size for all families/partitions, and criteria
...
# CALL FORMAT:
#########################################################################################
# $OID_HOME/bin/snpgenie_oid_calc_between_group.pl <codon_min_taxa_total> <codon_min_taxa_group> <sw_size> <sw_step_size> <num_bootstraps> <procs_per_node>
#########################################################################################

#########################################################################################
# EXAMPLE CALLS:
#########################################################################################
# snpgenie_oid_calc_between_group.pl --sliding_window_size=8 --sliding_window_step=1 --num_bootstraps=1000 --procs_per_node=4
# snpgenie_oid_calc_between_group.pl --sliding_window_size=1 --sliding_window_step=1 --num_bootstraps=1000 --procs_per_node=1 --codon_min_taxa_total=5 --codon_min_taxa_group=2
#########################################################################################
...
my $between_group_partition_results_file = 'between_group_results_filtered.txt';
...
##	} els
    ...
    {    # DO IT
        ...
        if ( -f "$between_group_sw_output_file" ) {    # file already exists
            warn
"\n### WARNING: $between_group_sw_output_file already exists for family $family_id.\n"
...
"analysis\tfamily\tpartition\tgroup_1\tgroup_2\twindow\tfirst_codon\tlast_codon\t"
        ...
        ##################################################################################
        # READY THE OUTPUT FILE FOR PARTITION
        if ( -f "between\_group\_results\_filtered\.txt" )
        {    # file already exists
            warn
"\n### WARNING: between\_group\_results\_filtered\.txt already exists for family $family_id.\n"
              . "### REMOVING AND REPLACING using current criteria.\n";

            unlink "between\_group\_results\_filtered\.txt";
        }

        open( BETWEEN_OUTFILE, ">>between\_group\_results\_filtered\.txt" );
...
##		my $between_product_S_diffs_sum = 0;
        ...
        ##################################################################################
        # LOOP CODONS TO STORE DATA IN %between_group_partition_codon_data_hh
...
#					$line_arr[$header_indices{'N_sites'}] . "\n\n";
                ...
                # Sum partition values here, NONFILTERED
...
"\n\n### WARNING:\n## For partition $this_partition\,\n## there are "
                    ...
                    for (
                        my $codon_index = 0 ;
                        $codon_index <= $num_sorted_codons ;
                        $codon_index++
                      )
                    {
                        my $codon_num = $codon_index + 1;

                        my $num_defined_codons_g1 =
                          $between_group_partition_codon_data_hh{$this_analysis}
                          ->{$this_family}->{$this_partition}->{$codon_num}
                          ->{'num_defined_codons_g1'};
                        my $num_defined_codons_g2 =
                          $between_group_partition_codon_data_hh{$this_analysis}
                          ->{$this_family}->{$this_partition}->{$codon_num}
             ...
             #						print "\nnum_defined_codons_g1 is $num_defined_codons_g1\n";
                        ...
                        if (   $codon_min_taxa_total > 0
                            && $codon_min_taxa_group > 0 )
                        {    # BOTH criteria have been specified
                            my $taxa_total =
                              $num_defined_codons_g1 + $num_defined_codons_g2;
                            ...
                            if (   $taxa_total >= $codon_min_taxa_total
                                && $taxa_group_min >= $codon_min_taxa_group )
                            {    # see if criteria are met by this codon
                                $between_product_N_sites_sum_filtered +=
                                  $between_group_partition_codon_data_hh{
                                    $this_analysis}->{$this_family}
                                  ->{$this_partition}->{$codon_num}->{N_sites};
                                $between_product_S_sites_sum_filtered +=
                                  $between_group_partition_codon_data_hh{
                                    $this_analysis}->{$this_family}
                                  ->{$this_partition}->{$codon_num}->{S_sites};
                                $between_product_N_diffs_sum_filtered +=
                                  $between_group_partition_codon_data_hh{
                                    $this_analysis}->{$this_family}
                                  ->{$this_partition}->{$codon_num}->{N_diffs};
                                $between_product_S_diffs_sum_filtered +=
                                  $between_group_partition_codon_data_hh{
                                    $this_analysis}->{$this_family}
                                  ->{$this_partition}->{$codon_num}->{S_diffs};
                            }
                        }
                        elsif ( $codon_min_taxa_total > 0 )
                        {    # only TOTAL min has been specified
                            my $taxa_total =
                              $num_defined_codons_g1 + $num_defined_codons_g2;

                            if ( $taxa_total >= $codon_min_taxa_total )
                            {    # see if criterion is met by this codon
                                $between_product_N_sites_sum_filtered +=
                                  $between_group_partition_codon_data_hh{
                                    $this_analysis}->{$this_family}
                                  ->{$this_partition}->{$codon_num}->{N_sites};
                        ...
                        elsif ( $codon_min_taxa_group > 0 )
                        {    # only GROUP min has been specified
                            my $taxa_group_min = $num_defined_codons_g1;
                            ...
                            if ( $taxa_group_min >= $codon_min_taxa_group )
                            {    # see if criterion is met by this codon
                                $between_product_N_sites_sum_filtered +=
                                  $between_group_partition_codon_data_hh{
                                    $this_analysis}->{$this_family}
...
#					print "\nANALYSIS $this_analysis FAMILY $this_family PARTITION $this_partition\n".
...
#								print "\nTEST RETRIEVAL:\nanalysis: $this_analysis\nfamily: $this_family\npartition: $this_partition\n\n";
                                ...
                                if (   $codon_min_taxa_total > 0
                                    && $codon_min_taxa_group > 0 )
                                {    # BOTH criteria have been specified
                                    my $taxa_total = $num_defined_codons_g1 +
                                      $num_defined_codons_g2;
                                    my $taxa_group_min = $num_defined_codons_g1;

                                    if ( $num_defined_codons_g2 <
                                        $taxa_group_min )
                                    {
                                        $taxa_group_min =
                                          $num_defined_codons_g2;
...
"\t$SE_dN_minus_dS\t$window_boot_Z\t$significance\n"
                    ...
                    if (   $between_product_dS_filtered > 0
                        && $between_product_dS_filtered ne '*' )
                    {
                    ...
                    if (   $between_product_dN_filtered >= 0
                        && $between_product_dS_filtered >= 0 )
                    {
                        $between_product_dN_minus_dS_filtered =
                          $between_product_dN_filtered -
                          $between_product_dS_filtered;
                    }

                    my $out_line =
"$this_analysis\t$this_family\t$this_partition\t$group_name_i\t$group_name_j\t$between_product_N_sites_sum_filtered\t"
                      . "$between_product_S_sites_sum_filtered\t$between_product_N_diffs_sum_filtered\t$between_product_S_diffs_sum_filtered\t"
                      . "$between_product_dN_filtered\t$between_product_dS_filtered\t$between_product_dN_minus_dS_filtered\t$between_product_w_filtered";

                    # PRINT OUT RE-CALCULATED PARTITION VALUES
                    open( BETWEEN_OUTFILE,
                        ">>between\_group\_results\_filtered\.txt" );
