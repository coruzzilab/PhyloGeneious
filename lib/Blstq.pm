...
#		    "$BLASTALL -a $numProc -p blastp -d $sp -I -e $eCutOff -m 8 -i $tmpFile";
    ...
    open( XX, ">>$Blastdir/.Parts.done" );                #add to done file
        print XX ".$prefix.done\n";
    close XX;
}    ## runBlast
...
sub combBlast {
    my $noparts = shift;
    my $cntrec  = 0;
    open( FB, ">$Blastdir/$COMBOUT" )
      or do { print "open for $COMBOUT fails\n"; return };
    for ( my $part = 1 ; $part <= $noparts ; $part++ ) {
        foreach my $sp (@dbfiles) {
            open FA, "$Blastdir/$sp.Part$part.blst"
              or
              do { print "no $sp.Part$part.blast found.. skipping \n"; next };
            while (<FA>) {
                $cntrec++;
                print FB $_;
            }
