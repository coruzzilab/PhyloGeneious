...
## Proc files for tnt 
our %procptr = ();  #entry is cutoff, points to array with proc or hash ncpu with array of arrays
...
sub initOID {
	...
	while (<CONF_FH>) {
        ...
        elsif (/^\s*TNTB\s*=\s*(.+?)\s*$/) {       #intermediate tnt group cutoff and nthreads
             $_      = $1;
             my @tmp = m/(\d+)/g;
             $TNTB = $tmp[0];
             $TNTB_THR = $tmp[1];
...
#    print @limkey," limkey\n";
    ...
    if (! $#limkey) { #empty
        ...
        print "no limit entered useing 500,12,12 (12 gb,12 hours)\n";
    }else {
...
#	    foreach my $sp (@INGROUP, @OUTGROUP) {
        for (my $part=1;$part<=$Parts;$part++){ # each part has a unique set of source genes
            $ct = localtime;
            %blastRes = ();
...
#				my $spBlastResDB = "$OID_BLASTDIR/Part$part" . "_blastres.dbm";
				...
				if ( ! -f $spblstfl ) {
					print STDERR "Blast results for $sp.Part$part not found!\n";
					next;
				}
                open FH, "$spblstfl" or print "blast failed for $spblstfl\n";
                my $oldlocus = '';
                my %tseen = ();
                while (<FH>){
                    my @stuff = split;
...
#                    $spBlastRes{$locus} .= $_;
            ...
            foreach $locus( keys %blastRes) {
              print FA ">$locus\n$blastRes{$locus}";  #i think there are imbeded c/r
              $count ++;
             }
         }
        close FA;
...
#	close FA;
	...
	foreach my $sp ( @INGROUP, @OUTGROUP ) {
        ...
        if ( -f "$OID_BLASTDIR/.$sp.Part$prefix.done"){
...
#                "$BLASTALL -db $sp -query $tmpFile -outfmt 6 -evalue $eCutOff "
#                . "-num_threads $numProc  -out $outfile";
		my $blastOutput = `$blastCmd`;
        $t = localtime;
        print "$t complete blast $sp rc $blastOutput\n" if $verbose>1;
        open(XX,">$OID_BLASTDIR/.$sp.Part$prefix.done");  #create a done file
        close XX;
...
sub mclmcx() {
    ...
    if ($bllen < 1000) { #gigabyte
	    ...
	    open(MFH, "|mcxload -abc - --write-binary -ri max -re max -o $mciFile -write-tab $tabFile");
    }
...
#		my @blastOutput = split( /\n/, $blastRes{$g});
	...
	my @mclArgs = ($mciFile, "-force-connected", "y", "-scheme", "7", "-I", "1.4", "-use-tab", $tabFile, "-o", $clustersFile);
...
#	my $FASTACMD = defined($BLAST_HOME) ? "$BLAST_HOME/bin/fastacmd" : "fastacmd";
	...
	while (<MFH>) {
		chomp;
	        if ( -f "$OID_DATADIR/.$familyNum.fam.done") {
			$familyNum++;
			next;
        	}
		my $seq;
		...
		if (@genes == 1) {
			...
			foreach my $sps (keys %spsInFam){
				...
				close TMPIDS;
				@out = `$FASTACMD -db $sps -entry_batch tempIDs -outfmt '>%a %s'`;
...
#
        ...
        if (!$curcut){
            ...
            print "$ntaxa greater that largest cutoff $curcut max $maxcut - family probably too large\n";
        }
...
my $grouphi = 0; #highest dir to run in group -- note grouplo has larger family size (lower group, bigger family
my $ngroup = 0;  #count of directories in group (may be missing dir between grouplo and grouphi
...
#    my @taxsz = sort { $sztaxa{$a} <=> $sztaxa{$b} } @kefnd;
    ...
    for $fam (@taxsz) {
        ...
        $nbigmon +=1 if $thissz > $TNTB;  # this is # of direcories sched by Bigmon
...
#        }
    ...
    if ($ngroup) {
        ...
        print "there are $ngroup families between $grouplo and $grouphi avg family: $tgroupsz\n";
        if ($qsgrp ){
...
#    for $fam (@ntax)  #smaller fam is larger Family size
    ...
    for (my $i=0; $i <= $#taxsz;$i++){
        ...
        if ($thissz > $TNTB){
            if ($thissz >= $TNTD) {
                print "family $fam has size $thissz - we think this is too big - skipping\n";
                next;
            }
