...
sub readFasta(@) {
	...
	foreach my $file (@fastaFile) {
		...
		if ($file eq "-") {
			...
			open $fh, "$file" or warn "readFasta: Error opening FASTA file \"$file\": $!\n";
		}
...
sub gap2missing($$) {
	...
	$tempStr =~ s/-*$//;
	$lenDiff = $seqLen - length($tempStr);
	$seq = substr($seq, 0, length($tempStr)) . $missingChar x $lenDiff if $lenDiff > 0;
...
sub genSuperMatrix {
	...
	$matrix .= "FORMAT INTERLEAVE SYMBOLS=\"ABCDEFGHIKLMNPQRSTUVWXYZ\" DATATYPE=PROTEIN ";
	$matrix .= "MISSING=$missingChar GAP=-;\n";
	$matrix .= "MATRIX\n";
	foreach my $i (0..$setNum-1) {
		my $anno = $$annoRef[$i];
		$anno =~ /^[^|]+/;
		my $partID = $&;      # assume first pipe-delimited field of annotation is partition id
		#my $partID = "OG" . ($i+1);
		$matrix .= "\n[ $anno|chars " . $setRange[$i] . " ]\n";
		my %currSet = %{$sets[$i]};
		my $partSize = 0;
		foreach my $taxon (@taxa) {
			$matrix .= "$taxon" . " " x ($maxNameLen-length($taxon)+1) . $currSet{$taxon} . "\n";
...
sub profileFit($$) {
	...
	if ($noMissing ne $profile) {
		open (NFH, ">$noMissingFile") or
			die "Cannot open $noMissingFile for writing: $!";
		print NFH "$noMissing";
		close NFH;
...
#	if (!defined($uninf)) {
		...
		while (<GFH>) {
			...
			my $parenTree = $1;
			if ($parenTree =~ /^(\(\([^)]+\),[^,)]+\)|\([^,)]+,\([^)]+\)\))$/) {
				print STDERR "guide tree $parenTree equivalent to all trees ... will not use constraint\n";
				$uninf = 1;
			}
			s/[^(]//g;
			if (length == 1) {
				print STDERR "guide tree $parenTree uninformative ... will not use contraint\n";
				$uninf = 1;
			}
		}
#	}
	...
	if (defined $uninf) {      # uninformative guide tree, do swap
		...
		if (@taxa <= 10) {     # exact search
			...
			if (@qNames > 1) {
				print NEX "hsearch addseq=random enforce=yes swap=nni rearrlimit=10000 limitperrep=yes;\n";
...
sub classifyQuery($$$$) {
	...
	foreach my $q (keys(%$unalignedQuery)) {
		...
		$augmentedAlignFile = "tmp.aln";  # alignment file augmented with this query
		...
		open GFH, $tempTreeFile or die "Cannot open guide tree \"$tempTreeFile\": $!\n";
		while (<GFH>) {
			if (/^\s*Translate/i) {
